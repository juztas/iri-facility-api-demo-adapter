...
from demo_adapter.combined import DemoAdapter
from demo_adapter.state import STATE
...
class StorageEndpointTests(unittest.TestCase):
    ...
    def test_access_endpoints_return_all_protocols_for_cfs(self):
        ...
        endpoints = asyncio.run(
            self.adapter.get_access_endpoints(cfs_resource, self.user, None, None)
        )
    ...
    def test_access_endpoints_filter_by_protocol(self):
        ...
        endpoints = asyncio.run(
            self.adapter.get_access_endpoints(cfs_resource, self.user, storage_models.AccessProtocol.globus, None)
        )
    ...
    def test_access_endpoints_filter_by_endpoint_id(self):
        ...
        endpoints = asyncio.run(
            self.adapter.get_access_endpoints(cfs_resource, self.user, None, "xrootd-cfs-demo")
        )
    ...
    def test_access_endpoints_hpss_has_only_globus(self):
        ...
        endpoints = asyncio.run(
            self.adapter.get_access_endpoints(hpss_resource, self.user, None, None)
        )
    ...
    def test_access_endpoints_unknown_resource_returns_empty(self):
        ...
        endpoints = asyncio.run(
            self.adapter.get_access_endpoints(fake_resource, self.user, None, None)
        )
    ...
    def test_openapi_exposes_access_endpoints_path(self):
        prefix = f"/{config.API_URL}"
        path = self.openapi["paths"][f"{prefix}/storage/access-endpoints/{{resource_id}}"]["get"]
        self.assertTrue(
